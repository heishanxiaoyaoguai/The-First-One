...
class ConfigHttp:
    ...
    def set_files(self, filename):
        ...
        """
        if filename != '':
            file_path = 'D:/image/' + filename
            #cself.files = {'file': open(file_path, 'rb')}
            self.files = {'file': (filename, open(file_path, 'rb'), 'image/jpeg')}
    ...
    def postWithFile(self):
        ...
        try:
            print(self.url)
            print(self.files)
            response = requests.post(self.url, headers=self.headers, data=None, files=self.files, timeout=float(timeout))
            return response
        except TimeoutError:
            self.logger.error("Time out!")
            return None
